// feature-previews.jsx — Animated feature showcase components
// Wrapped in IIFE to avoid global scope collisions with main script
(function () {
  'use strict';
  ...
  function DemoFrame({ children, title }) {
    return (
      <div style={{ background: '#080810', border: '1px solid #2a2a38', borderRadius: 13, overflow: 'hidden', boxShadow: '0 36px 88px rgba(0,0,0,0.6), 0 0 0 1px rgba(240,160,32,0.06)' }}>
        <div style={{ padding: '9px 14px', background: S1, borderBottom: `1px solid ${BORDER}`, display: 'flex', alignItems: 'center', gap: 7 }}>
          ...
          <span style={{ fontSize:11, color:TEXT3, fontFamily:FF, marginLeft:5, fontWeight:500 }}>{title || 'Recon AI'}</span>
        </div>
        {children}
      </div>
    );
  ...
  function ScoreBadge({ score, sz }) {
    ...
    return (
      <div style={{ width:sz, height:sz, borderRadius:6, display:'flex', alignItems:'center', justifyContent:'center', fontSize:sz<=30?10:12, fontWeight:700, fontFamily:MONO, background:bg, color:col, border:`1px solid ${bor}`, flexShrink:0, transition:'all 0.45s ease' }}>{score}</div>
    );
  }

  // ── DEMO 1: Lead Discovery ───────────────────────────────────────────
  ...
  function LeadDiscoveryDemo() {
    ...
    useEffect(() => { const t = setTimeout(() => setPhase(0), 400); return () => clearTimeout(t); }, []);
    ...
    useEffect(() => {
      ...
      if (phase === 0) {
        setTyped(0); setCards(0);
        timers.push(setTimeout(() => setPhase(1), 500));
      }
      if (phase === 1) {
        let i = 0;
        const go = () => { i++; setTyped(i); if (i < SEARCH_Q.length) timers.push(setTimeout(go, 46)); else timers.push(setTimeout(() => setPhase(2), 480)); };
        timers.push(setTimeout(go, 60));
      }
      if (phase === 2) { timers.push(setTimeout(() => setPhase(3), 1100)); }
      if (phase === 3) {
        [0,340,680,1020].forEach((d,i) => timers.push(setTimeout(() => setCards(i+1), d)));
        timers.push(setTimeout(() => setPhase(4), 2700));
      }
      if (phase === 4) { timers.push(setTimeout(() => setPhase(0), 2000)); }
      ...
      <DemoFrame title="Lead Discovery — Plumbers · Austin, TX">
        <div style={{ padding:14, background:'#080810' }}>
          ...
          <div style={{ background:S2, borderRadius:9, border:`1px solid ${phase>=1 ? 'rgba(240,160,32,0.4)' : BORDER}`, display:'flex', overflow:'hidden', marginBottom:11, transition:'border-color 0.3s' }}>
            ...
            <div style={{ background: typed>0 ? AMBER : '#2a2a38', color: typed>0 ? '#0b0b0f' : TEXT3, padding:'0 16px', display:'flex', alignItems:'center', fontSize:12, fontWeight:700, fontFamily:FF, flexShrink:0, transition:'background 0.3s, color 0.3s' }}>Search</div>
          </div>
              ...
              return (
                <div key={i} style={{ background:S2, border:`1px solid ${BORDER}`, borderRadius:8, padding:'11px 12px', opacity:show?1:0, transform:show?'translateY(0)':'translateY(10px)', transition:'opacity 0.38s ease, transform 0.38s ease' }}>
                  <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', marginBottom:5 }}>
                    <div style={{ fontSize:11, fontWeight:700, color:TEXT, flex:1, marginRight:6, lineHeight:1.35 }}>{l.name}</div>
                    <ScoreBadge score={l.score} sz={28} />
                  </div>
                  <div style={{ fontSize:10, color:TEXT3, marginBottom:6, fontFamily:FF }}>{l.addr}</div>
                  <div style={{ display:'flex', gap:3, flexWrap:'wrap' }}>
                    {l.issues.map((iss,j)=><span key={j} style={issueStyle(iss.t)}>{iss.l}</span>)}
                  </div>
                </div>
              );
            })}
  ...
  function PipelineDemo() {
    ...
    useEffect(() => {
      const iv = setInterval(() => {
        setFading(true);
        const t = setTimeout(() => { setCol(c => (c+1)%KCOLS.length); setFading(false); }, 380);
            ...
            return (
              <div key={kc.id} style={{ flex:1, minWidth:0, display:'flex', flexDirection:'column' }}>
                <div style={{ padding:'7px 9px', background:S2, border:`1px solid ${BORDER}`, borderBottom:'none', borderRadius:'7px 7px 0 0', display:'flex', alignItems:'center', justifyContent:'space-between' }}>
                  <span style={{ fontSize:9, fontWeight:700, textTransform:'uppercase', letterSpacing:'0.09em', color:colColor, fontFamily:FF }}>{kc.label}</span>
                  <span style={{ fontSize:9, fontFamily:MONO, color:TEXT3, background:S3, padding:'1px 5px', borderRadius:3 }}>{statics.length + (movingHere ? 1 : 0)}</span>
                </div>
                <div style={{ flex:1, background:'rgba(255,255,255,0.01)', border:`1px solid ${isActive ? colColor+'55' : BORDER}`, borderTop:'none', borderRadius:'0 0 7px 7px', padding:6, display:'flex', flexDirection:'column', gap:5, minHeight:148, transition:'border-color 0.4s' }}>
                  {statics.map((nm,si)=>(
                    <div key={si} style={{ background:S2, border:`1px solid ${BORDER}`, borderRadius:5, padding:'7px 8px' }}>
                      <div style={{ fontSize:9.5, fontWeight:700, color:TEXT2, fontFamily:FF, lineHeight:1.3 }}>{nm}</div>
                    </div>
                  ))}
                  {movingHere && (
                    <div style={{
                      ...
                      <div style={{ fontSize:9.5, fontWeight:700, color:TEXT, fontFamily:FF }}>{KMOVING.name}</div>
                      <div style={{ fontSize:9, color:TEXT3, fontFamily:MONO, marginTop:2 }}>{KMOVING.phone}</div>
                    </div>
                  )}
                </div>
              </div>
  ...
  function ScriptsDemo() {
    ...
    useEffect(() => {
      ...
      const typeStep = (idx, next) => {
        setShown(idx+1);
        const text = STEPS[idx].body;
        let i = 0;
        const go = () => {
          if (cancelled) return;
          i++; setChars(prev => { const n=[...prev]; n[idx]=i; return n; });
      ...
      if (phase === 0) {
        setShown(0); setChars([0,0,0]); setCopied(false);
        timers.push(setTimeout(() => setPhase(1), 400));
      }
      if (phase === 1) { typeStep(0, () => setPhase(2)); }
      if (phase === 2) { typeStep(1, () => setPhase(3)); }
      if (phase === 3) {
        typeStep(2, () => {
      ...
      if (phase === 4) { timers.push(setTimeout(() => setPhase(0), 1600)); }
      return () => { cancelled = true; timers.forEach(clearTimeout); };
    }, [phase]);
              ...
              <div key={i} style={{ background:S2, border:`1px solid ${BORDER}`, borderRadius:9, overflow:'hidden', opacity:show?1:0, transform:show?'translateY(0)':'translateY(9px)', transition:'opacity 0.4s ease, transform 0.4s ease' }}>
                <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', padding:'9px 13px', borderBottom:`1px solid ${BORDER}`, background:'rgba(255,255,255,0.018)' }}>
                  <div style={{ display:'flex', alignItems:'center', gap:9 }}>
                    <div style={{ width:20, height:20, borderRadius:'50%', background:'rgba(240,160,32,0.12)', border:'1px solid rgba(240,160,32,0.25)', color:AMBER, fontSize:10, fontWeight:700, fontFamily:MONO, display:'flex', alignItems:'center', justifyContent:'center', flexShrink:0 }}>{step.n}</div>
                    <div>
                      <div style={{ fontSize:11.5, fontWeight:700, color:TEXT, fontFamily:FF }}>{step.title}</div>
                      <div style={{ fontSize:9.5, color:TEXT3, fontFamily:FF, marginTop:1 }}>{step.hint}</div>
                    </div>
                  </div>
                  {done && (
                    <button style={{ background: copied&&isLast ? 'rgba(34,197,94,0.12)' : S3, border:`1px solid ${copied&&isLast ? 'rgba(34,197,94,0.3)' : B2}`, color: copied&&isLast ? GREEN : TEXT2, fontSize:10, fontWeight:700, padding:'4px 9px', borderRadius:5, cursor:'pointer', fontFamily:FF, transition:'all 0.2s' }}>
  ...
  // ── DEMO 4: Lead Scoring ─────────────────────────────────────────────
  const ISSUE_TAGS = [
    { l:'No HTTPS',   t:'crit' }, { l:'Slow load', t:'crit' },
    { l:'No mobile',  t:'warn' }, { l:'No GMB',    t:'warn' },
    { l:'Old design', t:'info' }, { l:'No contact',t:'info' },
  ];
  ...
  function ScoringDemo() {
    ...
    useEffect(() => { const t = setTimeout(() => setPhase(0), 500); return () => clearTimeout(t); }, []);
    ...
    useEffect(() => {
      ...
      if (phase === 0) {
        setScore(0); setStats([0,0,0]); setTags(0);
        timers.push(setTimeout(() => setPhase(1), 400));
      }
      if (phase === 1) {
      ...
      if (phase === 2) { timers.push(setTimeout(() => setPhase(0), 2000)); }
      return () => { cancelled = true; timers.forEach(clearTimeout); };
    }, [phase]);
    ...
    const tagC   = (t) => t==='crit' ? {bg:'rgba(239,68,68,0.1)',col:RED,bor:'rgba(239,68,68,0.22)'} : t==='warn' ? {bg:'rgba(240,160,32,0.1)',col:AMBER,bor:'rgba(240,160,32,0.2)'} : {bg:S3,col:TEXT2,bor:BORDER};
        ...
        <div style={{ padding:14, background:'#080810' }}>
          ...
          <div style={{ display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:8, marginBottom:10 }}>
            {[
              ...
              <div key={i} style={{ background:S2, border:`1px solid ${BORDER}`, borderRadius:9, padding:'12px 11px' }}>
                <div style={{ fontSize:9, fontWeight:700, textTransform:'uppercase', letterSpacing:'0.09em', color:TEXT3, fontFamily:FF, marginBottom:8 }}>{c.label}</div>
                <div style={{ fontSize:22, fontWeight:800, fontFamily:MONO, lineHeight:1, color:c.col, transition:'color 0.5s' }}>{c.pre||''}{c.val.toLocaleString()}{c.suf}</div>
              </div>
            ))}
          </div>
          ...
          <div style={{ background:S2, border:`1px solid ${BORDER}`, borderRadius:9, padding:'12px 13px', marginBottom:10 }}>
            <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:10 }}>
              <div style={{ fontSize:9, fontWeight:700, textTransform:'uppercase', letterSpacing:'0.09em', color:TEXT3, fontFamily:FF }}>Overall Score</div>
              <div style={{ width:36, height:36, borderRadius:7, display:'flex', alignItems:'center', justifyContent:'center', fontSize:13, fontWeight:700, fontFamily:MONO, background:sBg, color:sColor, border:`1px solid ${sBor}`, transition:'all 0.5s ease' }}>{score}</div>
            </div>
          ...
          <div style={{ background:S2, border:`1px solid ${BORDER}`, borderRadius:9, padding:'11px 12px' }}>
            <div style={{ fontSize:9, fontWeight:700, textTransform:'uppercase', letterSpacing:'0.09em', color:TEXT3, fontFamily:FF, marginBottom:9 }}>Website Issues Detected</div>
            <div style={{ display:'flex', flexWrap:'wrap', gap:5 }}>
              {ISSUE_TAGS.map((tag,i)=>{
                const tc = tagC(tag.t);
                return (
                  <span key={i} style={{ fontSize:10, fontWeight:700, padding:'3px 7px', borderRadius:4, fontFamily:FF, background:tc.bg, color:tc.col, border:`1px solid ${tc.bor}`, opacity:tags>i?1:0, transform:tags>i?'scale(1)':'scale(0.82)', transition:'opacity 0.28s ease, transform 0.28s ease' }}>{tag.l}</span>
                );
              })}
            </div>
          </div>
  ...
  function ShowcaseRow({ label, title, desc, demo, reversed }) {
    ...
    return (
      <div ref={ref} style={{ borderTop:'1px solid #1a1a24', padding:'clamp(48px,7vh,80px) clamp(20px,5vw,72px)' }}>
        <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fit,minmax(280px,1fr))', gap:'clamp(32px,5vw,72px)', alignItems:'center', maxWidth:1160, margin:'0 auto' }}>
          <div style={{ ...textAnim, order: reversed ? 1 : 0 }}>
            <div style={{ fontSize:11, fontWeight:700, letterSpacing:'1.2px', color:'#f0a020', textTransform:'uppercase', fontFamily:FF, marginBottom:14 }}>{label}</div>
            <h3 style={{ fontFamily:FF, fontWeight:800, fontSize:'clamp(22px,3.2vw,38px)', color:'#edeef2', letterSpacing:'-0.028em', lineHeight:1.16, marginBottom:16 }}>{title}</h3>
            <p style={{ fontFamily:FF, fontSize:'clamp(14px,1.3vw,16px)', color:'#888898', lineHeight:1.78 }}>{desc}</p>
          </div>
          <div style={{ ...demoAnim, order: reversed ? 0 : 1 }}>{demo}</div>
        </div>
      </div>
    );
  }

  // ── FeaturesShowcase (public) ─────────────────────────────────────────
  function FeaturesShowcase() {
    return (
      <div style={{ position:'relative', zIndex:1 }}>
        {/* Section header */}
        <div style={{ textAlign:'center', padding:'clamp(56px,9vh,96px) 20px 0', borderTop:'1px solid #1a1a24' }}>
          <div style={{ fontSize:11, fontWeight:700, letterSpacing:'1.5px', color:'#f0a020', textTransform:'uppercase', fontFamily:FF, marginBottom:14 }}>See it in action</div>
  ...
  window.FeaturesShowcase = FeaturesShowcase;
})();
